...
logis_model = LogisticRegression()  # 0
neuralN_model = MLPClassifier(verbose=0)
svm_model = svm.SVC(kernel='linear', degree=1, probability=True, random_state=0)  # 0
dtree_model = DecisionTreeClassifier()
nv_model = GaussianNB()
knn = KNeighborsClassifier()

lista = [logis_model]#rf_model, neuralN_model, dtree_model, nv_model, knn]
...
for aqui in lista:
    ...
    def fitness(genome: Genome, data: x_train, data_solution: y_train, test_data: x_test, test_solution: y_test, model: aqui) -> float:
    ...
    def run_evolution(
        ...
        for i in range(generation_limit):
            ...
            total_accuracy.append(best_accuracy)
        ...
        return population, i, total_accuracy

    start = time.time()

    population, generations, total_accuracy = run_evolution(
        populate_func=partial(
            generate_population, size=50, genome_length=len(x_train.columns)
        ),
        fitness_func=partial(
            fitness, data=x_train, data_solution=y_train, test_data=x_test, test_solution=y_test,  model=aqui
        ),
        fitness_limit=1,
        generation_limit=50
    )
    ...
    accuracy_final = accuracy_score(y_test, aqui.fit(np.array(population[0]).T * x_train,y_train).predict(x_test))
    print(aqui)
    print(f"number of generations: {generations}")
    print(f"Time: {end-start}s")
    print(f"best solution: {genome_to_things(population[0], things)}", aqui, '<--------------------------------')
    print("Final accuracy", accuracy_final)

    total_accuracy = [i * 100 for i in total_accuracy]
    total_accuracy = [round(num, 1) for num in total_accuracy]


    x = np.arange(1, (generations+2))
    fig = plt.figure(figsize=(8,5.25))
    #bax.set_yticks(np.r_[0:0.8:0.1, 0.8:1.05:0.05])
    bax = brokenaxes(ylims=((0, 1), (90, 100)), hspace=.1)
    bax.set_xlim([0, (generations + 1)])
    bax.plot(x, total_accuracy, c='orange', marker='o', markersize= 3)
    bax.set_title("Accuracy over generations")
    bax.set_xlabel("Number of generations")
